...
**YOLO11n-pose（ONNX / onnxruntime-web）** が、画面内の針の「根元」「針先」の2キーポイントを直接検出します。この2点から刺入角度・到達距離を直接計算しており、以前使用していたOpenCV.js(Canny/HoughLinesP)による画像処理ベースの直線検出は廃止しました（背景を誤って針と認識する問題が根本的に解消されます）。

YOLOモデルの読み込み・推論に失敗した場合、代替の検出手段はありません。その場合は画面に「AIモデルの読み込みに失敗しました」と表示されます。
...
├── icon-192.png
├── icon-512.png
├── best.onnx                      # 針検出モデル (YOLO11n-pose / 自前学習)
...
| モデル学習 | [Ultralytics YOLO11n-pose](https://github.com/ultralytics/ultralytics) |
...
- **`best.onnx`（同梱モデル）** — [Ultralytics YOLO11-pose](https://github.com/ultralytics/ultralytics)を用いて学習。UltralyticsのYOLOモデル・学習コードは **AGPL-3.0** で提供されており、学習済みモデルを含む本アプリを公開・配布する場合、AGPL-3.0の条件（ソースコード全体の同ライセンスでの公開等）が適用される可能性があります。商用利用やAGPL-3.0を避けたい場合は、[Ultralytics Enterprise License](https://www.ultralytics.com/license)の取得を検討してください。本READMEはライセンスの法的助言ではないため、正式な判断は原文および必要に応じて専門家にご確認ください。
